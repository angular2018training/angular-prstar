...
@Component({
  ...
  template: `
  <select #select style="width: 100%;" class="mdb-select" [disabled]="disabled">
    <option></option>
    <ng-template let-c ngFor [ngForOf]="allTimezones">
      <optgroup *ngIf="c.zones.length > 1" [label]="c.iso | iso2CountryPipe">
        <option *ngFor="let t of c.zones" [value]="outputOffsetTimezone(t)">{{c.iso | iso2CountryPipe}} - {{formatTimezoneString(t)}}
            <span *ngIf="showOffset">{{offsetOfTimezone(t)}}</span>
        </option>
      </optgroup>
      <option *ngIf="c.zones.length === 1" [value]="outputOffsetTimezone(c.zones[0])">{{c.iso | iso2CountryPipe}}
...
export class TimezonePickerComponent implements AfterViewInit {
  ...
  outputOffsetTimezone(zone: string): string {
    let offset = moment.tz(zone).utcOffset();
    const neg = offset < 0;
    if (neg) {
      offset = -1 * offset;
